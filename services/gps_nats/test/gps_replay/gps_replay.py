...
import time            # For sleeping between dbus messages
...
async def run(loop):
    ...
    def gps_to_dbus(dbusargs):
        if len(dbusargs) != 15:
            print('tuple argument for gps_to_dbus was not valid. Expected 15 entries, got', len(
                dbusargs))
            return -1
    ...
    if not os.path.isfile(testpointsloc):
        print('Test points file does not exist in',
              testpointsloc, ', cannot insert test points')
        exit(-1)
    ...
    # Loosely based off this example: https://docs.nats.io/developing-with-nats/sending/request_reply
    ...
    with open(testpointsloc) as f:
        for line in f:
            ...
            if point['class'] == 'TPV' and point['mode'] == 3:
                ...
                if timeraw[-1] == 'Z':
                    timeraw = timeraw[:-1]
                time_epoch = datetime.datetime.fromisoformat(
                    timeraw).timestamp()
                ...
                # Publish GPS point to NATS
                print('Publishing gps data')
                await nc.publish("gps", json.dumps(point).encode())

                # Messages are not pushed through right away if this is not here.
                # It still works but it makes the logs prettier. Theoretically it
                # also means that will end the test as soon as a sent gps point does
                # not appear instead of waiting until the end
                await nc.flush(1)
                sys.stdout.flush()
